    ...
    if ($(window).width() >= 768) {
      ...
      <div class="job-card row">
        ...
        `).join('')}
          ...
          <a class="view-more" onclick="showJobDetail('${hireTab[j]}',${index})" href="javascript:void(0);">View More -></a>          </p>
          </div>
        </div>
      </div>
      `);
...
function showJobDetail(selectedHireTab, index) {
  $("#hiringPage").css("display", "none");
  $("#detailPage").css("display", "block");
  scroll(0, 0)
  const job = jobs[selectedHireTab][index];
  $(".position-name").replaceWith(`
